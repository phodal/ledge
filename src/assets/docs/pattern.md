...
## GitOps
> GitOps是一种正在流行的DevOps最佳实践，最早是由Weaveworks公司与2017年提出。它结合基础设施即代码(IaC)，版本控制系统(通常是Git)，持续集成/持续交付(CI/CD)，将Git仓库作为声明基础设施和应用代码的唯一事实来源，并通过Pull Request(PR)来自动化地管理基础设施的开发和部署，并对整个过程的状态变化进行持续监控，审计等。
